...
Asset registration created two effects: (1) the state guard is now aware of assets' states, and (2) Ansible EDA playbooks are triggered by state-triggered events.
...
Your action is stopped by a guard built into the os_stop role, that requires proper state before actual stop. Operator tries to enforce STOPPED state by hacking state guard.
...
Again this action is rejected; this time by a state guard.
...
. State Guard should be extended by state guards to enforce business rules like maintenance window, dependencies, etc. The critical aspect is dependency, which will make it possible to model `service/nodes/resources` relationship.
...
. Finite State Machine (FSM) is not an orchestration engine (Turing Machine). It covers a very specialized function to control assets' states; however, it is not meant to control the logic flow.
